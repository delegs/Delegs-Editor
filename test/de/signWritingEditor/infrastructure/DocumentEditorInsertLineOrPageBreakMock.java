...
public class DocumentEditorInsertLineOrPageBreakMock extends DocumentEditor {

	private Document dummyDocument;
	private boolean automaticFreeTextLine;

	public DocumentEditorInsertLineOrPageBreakMock(DictionaryServiceAsync dictionaryService,
			DocumentServiceAsync documentService, PdfServiceAsync pdfService, VideoServiceAsync videoService,
			MediaUrlServiceAsync mediaUrlService, BrowserHistoryService browserHistoryService,
			FontSizeService fontSizeService, BadgeServiceClient badgeService, IdFactory idFactory,
			TextbasedTokenStyleFactory tokenStyleFactory, LocalSessionService localSessionService, RoomItem rootRoom,
			SignLocale defaultSignLocale, DocumentEditorListener listener, Document dummyDocument) {
	...
	public void setAtomaticFreeTextLine(boolean result) {
		automaticFreeTextLine = result;
	}
	...
	protected GWTDocumentLayouter getDocumentLayouter() {
		return new GWTDocumentLayouterMock();
	}

	@Override
	public boolean isAutomaticFreeTextLineEnabled() {
		return automaticFreeTextLine;
	}
